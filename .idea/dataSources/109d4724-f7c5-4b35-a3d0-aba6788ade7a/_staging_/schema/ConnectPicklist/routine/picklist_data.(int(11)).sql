DROP PROCEDURE picklist_data;
CREATE PROCEDURE sp_picklist_data(OUT `_file_data` TEXT, IN `_collection_id` INT)
  BEGIN
    ...
    SELECT
      CONCAT(node, '|', ROW_NUMBER() OVER(ORDER BY ( SELECT 1)), '|', tmn_ref_id, '|', is_parent, '|', type_short, '|',
             tmn_ref_id, '|', description, '|', description, '|', type_long, '|', CHAR(10))
    INTO _data
    FROM
      (
        SELECT
    ...
    SET _data = CONCAT(
        'T=6', CHAR(10),
        'I=', HEX(_collection_id), CHAR(10),
        'R=1', CHAR(10),
        'L=PICKDOC.TXT', CHAR(10),
        'M=d67c', CHAR(10),
        'D=', CHAR(10), _data
    );
    ...
    SET _data = CONCAT(_data, @small_files);
